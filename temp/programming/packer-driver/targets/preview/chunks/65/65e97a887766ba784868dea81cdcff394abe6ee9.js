    ...
    execute: function () {
      ...
      _export("ZumaCurvePath", ZumaCurvePath = (_dec = ccclass('ZumaCurvePath'), _dec2 = property(Node), _dec3 = property(Prefab), _dec(_class = (_class2 = class ZumaCurvePath extends Component {
        ...
        spawnBalls() {
          ...
          // }
          //生成spawncount组双球
          for (var i = 0; i < 2 * this.spawnCount; i++) {
            var _ball = null;

            if (i % 2 == 0) {
              _ball = instantiate(this.ballPrefab[i % this.ballPrefab.length]);
            } else {
              _ball = instantiate(this.ballPrefab[(i - 1) % this.ballPrefab.length]);
            }

            _ball.setParent(this.node);
        ...
        update(dt) {
          ...
          for (var i = 0; i < this.balls.length; i++) {
            var _ball2 = this.balls[i];
            if (!_ball2.isValid) continue;
            var t = this.ballPositions[i];
            ...
            if (t < 0) continue;
            var pos = this.getPointByT(t);
            _ball2.active = true;

            _ball2.setPosition(pos);
          }
        ...
        handleBulletCollision(bulletNode, hitNode) {
          ...
          var col = bulletNode.getComponent(Collider2D);
          col.group = 1 << 1;
          var hitIndex = this.balls.indexOf(hitNode);
          if (hitIndex < 0) return;
          var bulletBall = bulletNode.getComponent(_crd && ball === void 0 ? (_reportPossibleCrUseOfball({
            error: Error()
          }), ball) : ball);
          var hitBall = hitNode.getComponent(_crd && ball === void 0 ? (_reportPossibleCrUseOfball({
            error: Error()
          }), ball) : ball);
          if (!bulletBall || !hitBall) return;
          var insertT = (_this$ballPositions$h = this.ballPositions[hitIndex]) != null ? _this$ballPositions$h : 0; //0类型为插入
          ...
          if (!this.reconnecting) {
            ...
            if (hitIndex <= this.reconnectFrontEndIndex) {
              for (var _i = 0; _i <= hitIndex; _i++) {
                this.ballPositions[_i] += this.ballSpacing;
              }
            } else if (hitIndex >= this.reconnectBackStartIndex) {
              for (var _i2 = this.reconnectBackStartIndex; _i2 <= hitIndex; _i2++) {
                this.ballPositions[_i2] += this.ballSpacing;
              }
            }
          }
        ...
        syncBallNodesFromIndex(startIndex) {
          for (var i = startIndex; i < this.balls.length; i++) {
            var node = this.balls[i];
        ...
        resolveMatchFromIndex(index) {
          ...
          while (this.balls.length > 0 && checkIndex >= 0 && checkIndex < this.balls.length) {
            var runStart = this.findSameColorRunStart(checkIndex);
            if (runStart < 0) break;
            var runEnd = this.findSameColorRunEnd(checkIndex);
        ...
        updateReconnectGap(dt) {
          ...
          var step = this.moveSpeed * dt * 60;

          for (var i = 0; i <= this.reconnectFrontEndIndex; i++) {
            this.ballPositions[i] -= step;
          } // 只通过 ballPositions 判断后半段是否追上前半段


          var frontPos = this.ballPositions[this.reconnectFrontEndIndex];
          var backPos = this.ballPositions[this.reconnectBackStartIndex];
          var distanceAhead = frontPos - backPos;
        ...
        findSameColorRunStart(index) {
          ...
          var c = this.getBallColor(this.balls[index]);
          if (!c) return -1;
          var start = index;
        ...
        isSameColor(node, target) {
          var c = this.getBallColor(node);
          if (!c || !target) return false;
          return c.r === target.r && c.g === target.g && c.b === target.b && c.a === target.a;
        }

        getPointByT(t) {
          var segments = this.pathPoints.length - 1;
          if (segments < 1) return Vec3.ZERO;
          if (this.totalPathLength === 0) return this.pathPoints[0].position.clone();
          var targetDist = t * this.totalPathLength;

          if (targetDist < 0) {
            var p0 = this.pathPoints[0].position;
            var p1 = this.pathPoints[1].position;
            var dir = Vec3.subtract(new Vec3(), p0, p1);
            dir.normalize().multiplyScalar(-targetDist);
            return Vec3.add(new Vec3(), p0, dir);
          }

          if (targetDist > this.totalPathLength) {
            var _p = this.pathPoints[segments - 1].position;
            var _p2 = this.pathPoints[segments].position;

            var _dir = Vec3.subtract(new Vec3(), _p2, _p);

            _dir.normalize().multiplyScalar(targetDist - this.totalPathLength);

            return Vec3.add(new Vec3(), _p2, _dir);
          }

          for (var i = 0; i < segments; i++) {
            if (targetDist <= this.pathDistances[i + 1]) {
              var _p3 = this.pathPoints[i].position;
              var _p4 = this.pathPoints[i + 1].position;
              var dSeg = this.pathDistances[i + 1] - this.pathDistances[i];
              var localT = dSeg > 0 ? (targetDist - this.pathDistances[i]) / dSeg : 0;
              return Vec3.lerp(new Vec3(), _p3, _p4, localT);
            }
          }
