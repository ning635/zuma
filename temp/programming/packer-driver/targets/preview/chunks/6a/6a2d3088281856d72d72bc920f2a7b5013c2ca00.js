System.register(["cc"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "91023Wd4zVBD4BUze7wTIDK", "ZumaCurvePath", undefined);
      ...
      _export("ZumaPathManager", ZumaPathManager = (_dec = ccclass('ZumaPathManager'), _dec2 = property({
        type: [Node],
      ...
      }), _dec(_class = (_class2 = class ZumaPathManager extends Component {
        constructor() {
          super(...arguments);
        ...
        applyMobilePath() {
          ...
          var mobilePath = [new Vec3(-250, 410, 0), new Vec3(230, 410, 0), new Vec3(230, 245, 0), new Vec3(-215, 245, 0), new Vec3(-215, 75, 0), new Vec3(190, 75, 0), new Vec3(190, -92, 0), new Vec3(-160, -92, 0), new Vec3(-160, -248, 0), new Vec3(135, -248, 0), new Vec3(135, -390, 0), new Vec3(-20, -390, 0)];
          var count = Math.min(this.pathPoints.length, mobilePath.length);
        ...
        cachePathMetrics() {
          ...
          for (var i = 0; i < this.pathPoints.length - 1; i++) {
            var length = Vec3.distance(this.pathPoints[i].position, this.pathPoints[i + 1].position);
        ...
        spawnChain() {
          var colors = Math.max(1, this.ballPrefab.length);

          for (var i = 0; i < this.spawnCount; i++) {
            var color = Math.floor(i / 2) % colors;
            var kind = i > 0 && i % 7 === 0 ? 'relic' : 'normal';
            var distance = -i * this.ballSpacing;
        ...
        createChainBall(color, distance, kind) {
          ...
          var node = this.createBallNode(color, kind);
        ...
        createBallNode(color, kind) {
          if (kind === void 0) {
            kind = 'normal';
          }

          var prefab = this.ballPrefab[this.normalizeColor(color)];
          var node = prefab ? instantiate(prefab) : new Node("Ball_" + color);
          node.name = kind === 'relic' ? "RelicBall_" + this.normalizeColor(color) : "Ball_" + this.normalizeColor(color);
          var scale = kind === 'relic' ? RELIC_SCALE : BALL_SCALE;
        ...
        keepSpacing() {
          for (var i = 1; i < this.chain.length; i++) {
            ...
            var desired = previous.distance - this.ballSpacing;
        ...
        advanceShots(dt) {
          for (var i = this.shots.length - 1; i >= 0; i--) {
            ...
            var insertIndex = this.findHitIndex(shot.node.position, shot.targetIndex);
        ...
        findHitIndex(position, preferredIndex) {
          ...
          var bestIndex = -1;
          var bestDistance = this.hitRadius;

          for (var i = 0; i < this.chain.length; i++) {
            var distance = Vec3.distance(position, this.chain[i].node.position);
        ...
        insertShot(shotIndex, hitIndex) {
          ...
          var hitBall = this.chain[hitIndex];
          var insertDistance = hitBall.distance + this.ballSpacing;
          var inserted = this.createChainBall(shot.color, insertDistance, 'normal');
        ...
        resolveMatches(centerIndex) {
          ...
          var color = this.chain[centerIndex].color;
          ...
          var bridgeIndex = Math.min(start, this.chain.length - 1);
        ...
        resolveBomb(hitIndex) {
          var start = Math.max(0, hitIndex - 2);
          var end = Math.min(this.chain.length - 1, hitIndex + 2);
          this.combo = 0;
          this.score += 350;
          this.removeRange(start, end, true);
        ...
        removeRange(start, end, countRelics) {
          ...
          var safeEnd = Math.min(end, this.chain.length - 1);
          var anchorDistance = this.chain[start].distance;
          var removed = this.chain.splice(start, safeEnd - start + 1);

          for (var ball of removed) {
        ...
        compactAfterRemoval(startIndex, anchorDistance) {
          for (var i = startIndex; i < this.chain.length; i++) {
            var desired = anchorDistance - (i - startIndex) * this.ballSpacing;
        ...
        onTouchEnd(event) {
          ...
          var touchTarget = this.touchToLocalPosition(event);
          var assisted = this.findAimTarget(touchTarget);
          var fireTarget = assisted.index >= 0 ? this.chain[assisted.index].node.position.clone() : touchTarget;
          var direction = fireTarget.subtract(this.shooterPosition);
          ...
          direction.normalize();
          var node = this.createBallNode(this.currentColor);
        ...
        touchToLocalPosition(event) {
          ...
          var transform = parent == null ? void 0 : parent.getComponent(UITransform);
        ...
        findAimTarget(touchTarget) {
          var aim = touchTarget.clone().subtract(this.shooterPosition);
          ...
          for (var i = 0; i < this.chain.length; i++) {
            var toBall = this.chain[i].node.position.clone().subtract(this.shooterPosition);
            var forward = Vec3.dot(toBall, aim);

            if (forward <= 35) {
              continue;
            }

            var closest = this.shooterPosition.clone().add(aim.clone().multiplyScalar(forward));
            var distanceToRay = Vec3.distance(this.chain[i].node.position, closest);
            var colorBonus = this.chain[i].color === this.currentColor ? 18 : 0;
            var effectiveDistance = distanceToRay - colorBonus;
        ...
        onLose() {
          this.phase = 'lost';
          this.clearAimGuide();
          var message = this.rewardedReviveEnabled && !this.reviveUsed ? 'Tap to simulate rewarded revive: push back + bomb.' : 'Out of time. Tap to restart.';
        ...
        removePrototypeVisuals() {
          ...
          var names = ['TempleGardenBackdrop', 'MobileTempleBackdrop', 'MobileGameRoot', 'ReadableTrack', 'Shooter', 'AimGuide', 'PrototypeHUD', 'PrototypeHelper'];
        ...
        hideOldBackground() {
          ...
          var bg = (_this$node$parent3 = this.node.parent) == null ? void 0 : _this$node$parent3.getChildByName('BackGround0');
        ...
        createBackdrop() {
          ...
          var parent = (_this$node$parent4 = this.node.parent) != null ? _this$node$parent4 : this.node;
          var node = new Node('MobileGameRoot');
          node.setParent(parent);
          node.setSiblingIndex(1);
          this.backdropGraphics = node.addComponent(Graphics);
          var g = this.backdropGraphics;
          g.clear();
        ...
        drawTrack() {
          ...
          var parent = (_this$node$parent5 = this.node.parent) != null ? _this$node$parent5 : this.node;
          var trackNode = new Node('ReadableTrack');
          trackNode.setParent(parent);
          trackNode.setSiblingIndex(2);
          this.trackGraphics = trackNode.addComponent(Graphics);
          var g = this.trackGraphics;
          g.clear();
        ...
        createShooterUi() {
          ...
          var parent = (_this$node$parent6 = this.node.parent) != null ? _this$node$parent6 : this.node;
          ...
          var base = this.shooterNode.addComponent(Graphics);
          ...
          var helperNode = new Node('PrototypeHelper');
        ...
        drawAimGuide(touchTarget) {
          ...
          var assisted = this.findAimTarget(touchTarget);
          var end = assisted.index >= 0 ? this.chain[assisted.index].node.position : touchTarget;
          var g = this.guideGraphics;
          g.clear();
          g.lineWidth = 4;
        ...
        replacePreview(holder, color, position, scale) {
          ...
          var preview = this.createBallNode(color);
        ...
        refreshHud(extra) {
          ...
          var status = this.phase === 'playing' ? 'PLAY' : this.phase.toUpperCase();
          var bomb = this.nextShotType === 'bomb' ? 'READY' : this.streakCharge + "/3";
        ...
        pickHelpfulColor() {
          var colorCount = Math.max(1, this.ballPrefab.length);
          ...
          if (candidates.size > 0 && Math.random() < 0.62) {
            var weighted = Array.from(candidates.entries()).sort((a, b) => b[1] - a[1]);
        ...
        getPointByDistance(distance) {
          ...
          var remaining = Math.max(0, Math.min(this.pathLength, distance));

          for (var i = 0; i < this.segmentLengths.length; i++) {
            var length = this.segmentLengths[i];

            if (remaining <= length || i === this.segmentLengths.length - 1) {
              var localT = length <= 0 ? 0 : remaining / length;
              var p0 = this.pathPoints[i].position;
              var p1 = this.pathPoints[i + 1].position;
...
//# sourceMappingURL=6a2d3088281856d72d72bc920f2a7b5013c2ca00.js.map