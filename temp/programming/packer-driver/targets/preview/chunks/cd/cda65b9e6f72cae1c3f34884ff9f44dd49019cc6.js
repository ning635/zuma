    ...
    execute: function () {
      ...
      _export("control", control = (_dec = ccclass('control'), _dec2 = property({
        ...
        spawnPreview() {
          ...
          var prefab = this.balls[this.previewIndex];
          if (!prefab) return;
          ...
          var col = this.previewNode.getComponent(Collider2D);
          col.group = 1 << 2;
        ...
        update() {
          // 🔥 更新所有发射出的小球位置
          var dt = director.getDeltaTime();

          for (var i = this.activeBalls.length - 1; i >= 0; i--) {
            var _ball = this.activeBalls[i];

            if (_ball.node.isValid) {
              var pos = _ball.node.position;

              _ball.node.setPosition(pos.x + _ball.dir.x * this.shootSpeed * dt, pos.y + _ball.dir.y * this.shootSpeed * dt, pos.z); // 超出一定范围后自动销毁，防止内存泄漏
          ...
          var dx = this.mouseWorld.x - selfPos.x;
          var dy = this.mouseWorld.y - selfPos.y;
          ...
          if (this.graphics && this.drawNode.parent) {
            ...
            var uiTransform = this.drawNode.parent.getComponent(UITransform);

            if (uiTransform) {
              var localSelf = uiTransform.convertToNodeSpaceAR(selfPos);
              var localMouse = uiTransform.convertToNodeSpaceAR(this.mouseWorld); // 🔥 虚线样式全部按比例缩放

              var dashLen = 15 * this.lineScale;
              var spaceLen = 10 * this.lineScale;
              var currentLen = 0;
              var dir = new Vec3();
              Vec3.subtract(dir, localMouse, localSelf);
              dir.normalize();
              var distance = Vec3.distance(localSelf, localMouse);

              while (currentLen < distance) {
                var fromPos = new Vec3();
                var toPos = new Vec3();
                Vec3.scaleAndAdd(fromPos, localSelf, dir, currentLen);
                var nextLen = Math.min(currentLen + dashLen, distance);
              ...
              var crossSize = 10 * this.lineScale;
        ...
        shootBall(e) {
          ...
          var dx = targetWorld.x - selfPos.x;
          var dy = targetWorld.y - selfPos.y;
          var dir = new Vec3(dx, dy, 0);
